...
int main() {
    ...
    //Version 1
    cout << "\nDijkstra Version 1\n";
    dijkstraAll(g, 1, dist, pred);
    printResults(dist, pred);

    //Version 2
    cout << "\nDijkstra Version 2 (1 -> 5)\n";
    int distance = dijkstraOneDestination(g, 1, 5, pred);
    cout << "Shortest distance from 1 to 5: " << (distance == INF ? -1 : distance) << endl;

    cout << "Path: ";
    vector<int> path;
    for (int at = 5; at != -1; at = pred[at])
        path.push_back(at);
    ...
    //Version 3
    cout << "\nDijkstra Version 3\n";
    vector<int> destinations = {3, 4, 5};
    dijkstraMultiple(g, 1, destinations, dist, pred);
